...
server.registerTool(
  "set_flags",
  {
    title: "Set message flags",
    description:
      "Add or remove IMAP flags on a message, e.g. mark read/unread (\\Seen) or starred (\\Flagged).",
    inputSchema: {
      mailbox: z.string().default("INBOX"),
      uid: z.number().int().positive(),
      addFlags: z.array(z.string()).default([]).describe('e.g. ["\\\\Seen", "\\\\Flagged"]'),
      removeFlags: z.array(z.string()).default([]).describe('e.g. ["\\\\Seen"]'),
    },
  },
  async ({ mailbox, uid, addFlags, removeFlags }) => {
    try {
      assertMailboxAllowed(config, mailbox);
      await withClient(config, (client) =>
        withMailbox(client, mailbox, async () => {
          if (addFlags.length > 0) {
            await client.messageFlagsAdd({ uid: String(uid) }, addFlags, { uid: true });
          }
          if (removeFlags.length > 0) {
            await client.messageFlagsRemove({ uid: String(uid) }, removeFlags, { uid: true });
          }
        })
      );
      return text(`Updated flags on uid=${uid} in ${mailbox}`);
    } catch (err) {
      return errorResult(err);
    }
  }
...
server.registerTool(
  ...
  {
    title: "Move message",
    description: "Move a message from one mailbox to another (e.g. archive or file into a folder).",
    inputSchema: {
      mailbox: z.string().default("INBOX"),
      uid: z.number().int().positive(),
      targetMailbox: z.string().describe("Destination mailbox path, e.g. Archive"),
    },
  },
  async ({ mailbox, uid, targetMailbox }) => {
    try {
      ...
      await withClient(config, (client) =>
        withMailbox(client, mailbox, async () => {
          await client.messageMove({ uid: String(uid) }, targetMailbox, { uid: true });
        })
      );
      return text(`Moved uid=${uid} from ${mailbox} to ${targetMailbox}`);
    } catch (err) {
      return errorResult(err);
    }
  }
...
server.registerTool(
  "delete_message",
  {
    ...
    description:
      "Delete a message. By default marks it \\Deleted and expunges (permanent). Set permanent=false to just flag it \\Deleted without expunging.",
    inputSchema: {
      mailbox: z.string().default("INBOX"),
      uid: z.number().int().positive(),
  ...
  async ({ mailbox, uid, permanent }) => {
    try {
      ...
      return text(
        permanent
          ? `Permanently deleted uid=${uid} from ${mailbox}`
          : `Flagged uid=${uid} as \\Deleted in ${mailbox} (not expunged)`
      );
    } catch (err) {
      return errorResult(err);
    }
